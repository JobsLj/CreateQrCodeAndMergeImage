...
namespace CreateQrCodeAndMergeImage
{
    public class UpYun
    {
            ...
            else
            {
                request.Headers.Add("Authorization", "Basic " +
                                                     Convert.ToBase64String(
                                                         new System.Text.ASCIIEncoding().GetBytes(this.username + ":" +
                                                                                                  this.password)));
            }
        ...
        */
        ...
        public long getFolderUsage(string url)
        {
            ...
            try
            {
                ...
                var sr = new StreamReader(stream: resp.GetResponseStream(), encoding: Encoding.UTF8);
        ...
        */
        ...
        {
            this.auto_mkdir = autoMkdir;
            var headers = new Hashtable {{"folder", "create"}};
            var a = new byte[0];
            var resp = newWorker("POST", DL + this.bucketname + path, a, headers);
            if ((int) resp.StatusCode == 200)
            {
                resp.Close();
                return true;
        ...
        */
        ...
        public bool WriteFile(string path, byte[] data, bool autoMkdir)
        {
            var headers = new Hashtable();
            this.auto_mkdir = autoMkdir;
            HttpWebResponse resp = newWorker("POST", DL + this.bucketname + path, data, headers);
            if ((int) resp.StatusCode == 200)
            {
                resp.Close();
                return true;
        ...
        */
        ...
        public byte[] ReadFile(string path)
        {
            ...
            var resp = newWorker("GET", DL + this.bucketname + path, a, headers);
            if (resp != null)
            {
                // ReSharper disable once AssignNullToNotNullAttribute
                var sr = new StreamReader(stream: resp.GetResponseStream(), encoding: Encoding.UTF8);
        ...
        public Hashtable GetFileInfo(string file)
        {
            ...
            try
            {
                ht = new Hashtable
                {
                    ...
                    {"size", this.tmp_infos["x-upyun-file-size"]},
                    {"date", this.tmp_infos["x-upyun-file-date"]}
                };
            }
            catch (Exception)
            {
